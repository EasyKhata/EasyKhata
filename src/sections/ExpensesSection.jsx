...
export default function ExpensesSection({ year, month }) {
  ...
  function save() {
    ...
    if (form.recurring) payload.startMonth = form.date.slice(0, 7);
    else payload.month = form.date.slice(0, 7);
  ...
  const ExpRow = ({ expense }) => (
    <div className="card-row">
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12, color: "var(--text-dim)" }}>{expense.category}{expense.date ? ` - ${fmtDate(expense.date)}` : ""}{expense.note ? ` - ${expense.note}` : ""}</div>
      </div>
  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      ...
      {showForm && (
        <Modal title={editId ? "Edit Expense" : "Add Expense"} onClose={closeForm} onSave={save} canSave={!!form.label.trim() && Number(form.amount) > 0} accentColor="var(--danger)">
          ...
          {form.recurring && (
            <div style={{ background: "var(--blue-deep)", border: "1px solid var(--blue)33", borderRadius: 12, padding: "12px 14px", fontSize: 13, color: "var(--blue)", marginBottom: 16 }}>
              This expense will repeat from {MONTHS[new Date(form.date).getMonth()]} {new Date(form.date).getFullYear()} onward.
            </div>
          )}
